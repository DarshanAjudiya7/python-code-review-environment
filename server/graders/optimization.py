...
try:
    ...
    from .common import clamp_score, compile_tree, compiles, nested_loop_depth, style_score, syntax_error_message
    from .pytest_runner import run_pytest_suite
except ImportError:  # pragma: no cover
    from models import TaskGrade
    from task_bank import TaskSpec
    from graders.common import clamp_score, compile_tree, compiles, nested_loop_depth, style_score, syntax_error_message
...
def benchmark_runtime(candidate_code: str, task: TaskSpec) -> tuple[float, bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="python-code-review-bench-") as temp_dir:
        ...
        (temp_path / "candidate_runner.py").write_text(_benchmark_script("candidate", task), encoding="utf-8")
        (temp_path / "starter_runner.py").write_text(_benchmark_script("starter", task), encoding="utf-8")
...
def ast_quality_score(code: str, task: TaskSpec) -> float:
    ...
    function_node = next(
        (
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) and node.name == task.benchmark_entrypoint
        ),
        None,
    )
    ...
    marker_points = sum(0.05 for marker in task.expected_quality_markers if marker in code)
...
def grade_optimization_task(candidate_code: str, task: TaskSpec, include_hidden: bool = True) -> TaskGrade:
    ...
    score = clamp_score((0.55 * test_fraction) + (0.2 * runtime_score) + (0.15 * quality) + (0.10 * style))
    return TaskGrade(
        score=score,
        syntax_score=1.0,
        tests_passed=execution.passed,
