"""Environment implementation that works both in-repo and in server-only Docker builds."""
...
try:
    from .graders.syntax import grade_task
    from .models import (
        HealthResponse,
        HistoryEntry,
    ...
    from .task_bank import TaskSpec, get_task, list_task_summaries, task_ids
...
class PythonCodeReviewEnvironment(
    ...
    def _handle_analyze(self) -> tuple[RewardDetails, str]:
        ...
        reward = self._reward_from_grade(grade, self._previous_score, False, False, False, status)
    ...
    def _handle_edit(self, action: PythonCodeReviewAction) -> tuple[RewardDetails, str]:
        ...
        self._apply_grade_feedback(grade, include_hidden=False)
        status = self._build_status(grade, include_hidden=False)
        reward = self._reward_from_grade(grade, previous_score, True, True, False, status)
        self._append_history("edit_code", status, reward.value)
        self._previous_score = self._state.score
        return reward, status
    ...
    def _apply_grade_feedback(self, grade: TaskGrade, include_hidden: bool) -> None:
        ...
        self._state.test_results = self._format_test_results(grade, include_hidden)
    ...
    def _append_history(self, action_type: str, status: str, reward: float) -> None:
        self._state.history.append(
            HistoryEntry(step=self._state.step_count, action_type=action_type, status=status, reward=reward)
        )
