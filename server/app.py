"""FastAPI application that works both in-repo and in server-only Docker builds."""
...
try:
    ...
    from .models import HealthResponse, PythonCodeReviewAction, PythonCodeReviewObservation, PythonCodeReviewState, TaskGrade
...
def grade_task(task_id: str, payload: PythonCodeReviewAction) -> TaskGrade:
    if payload.action_type != "edit_code" or not payload.code:
        raise HTTPException(status_code=400, detail="Requires action_type='edit_code' with code parameter.")
    try:
